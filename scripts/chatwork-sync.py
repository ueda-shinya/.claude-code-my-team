# platform: win-only
# Windows 専用スクリプト（APScheduler で 1時間ごとに自動実行）。Mac からは実行しない。
"""
...
URGENT_AUTO_REPLY_TEXT = (
    'お世話になっております。\n'
    '担当の上田に代わり、AIアシスタントのアスカよりご連絡いたします。\n'
    '現在、ご連絡内容を確認しております。\n'
    '対応までしばらくお時間をいただいており、誠に恐れ入ります。\n'
    '上田より改めてご連絡いたしますので、今しばらくお待ちくださいませ。'
)
...
【全項目共通の大前提・例外なし】
本文に [To:{my_account_id}] または [toall] が含まれていない場合は、has_task / has_schedule / is_high_priority すべて必ず false を返すこと。
文脈からの推測・類推は一切禁止。「自分が対応者とわかる」「自分が関係者とわかる」という判断は認めない。
ただし、送信者が自分自身（account_id={my_account_id}）のメッセージについては、自分の送信内容に含まれる予定確定通知等を
拾って良い（この場合のみ [To:{my_account_id}] / [toall] の有無を問わない）。

なお、[toall] は全員宛メンションであるため、[To:{my_account_id}] と異なり「自分だけへの指示」ではない。
[toall] のみで宛先が指定されている場合は、後述の各判定セクションで定める「[toall] の追加条件」を満たす場合のみ true とすること。
また、[toall] は小文字固定の表記である。[ToAll] / [TOALL] 等の大文字・混在表記は Chatwork の実体と異なるため、宛先指定として扱わず false とすること。

【[rp aid=... to=...] タグに関する注意】
[rp aid=XXXX to=YYYY] の `to=` はChatworkの返信先メッセージIDであり、宛先の指定ではない。
`to=` に {my_account_id} が含まれていても、宛先指定（[To:{my_account_id}]）とは別物のため、has_task / has_schedule / is_high_priority の判定に使用してはならない。

has_task の判定基準（以下のいずれかに該当し、かつ大前提を満たす場合のみ true）：
- 本文に [To:{my_account_id}] があり、具体的な作業依頼がある場合
- 本文に [To:{my_account_id}] があり、クライアントからの制作・修正・更新依頼（画像差し替え、テキスト変更、ページ追加、バナー制作等）がある場合
- 本文に [To:{my_account_id}] があり、納品物の確認・承認依頼がある場合
- 本文に [To:{my_account_id}] があり、見積もり・提案書の作成依頼がある場合
- 本文に [toall] があり、かつ全員に対して具体的な作業・対応が明示的に求められている場合（例：「全員〇〇をやってください」「各自△△を確認して対応してください」等）
  ※ [toall] の場合の追加条件：単なる報告・情報共有・挨拶・議事録共有・周知のみは false。全員への対応要求が明確に読み取れる場合のみ true。
上記に該当しない場合は false。感謝・挨拶・雑談・完了済みの報告は false。
...
- 条件B: 自分（アカウントID: {my_account_id}）が参加・対応すべきイベントであること
  ...
  - 本文に [To:{my_account_id}] があること（文脈からの推測は禁止）
  - または、本文に [toall] があり、かつ全員参加が求められるMTG・締切等であること（[toall] の追加条件：単なる日程報告・議事録・過去の予定共有は false）
  - または、送信者が自分自身（account_id={my_account_id}）であり、予定が確定したことを報告・通知している場合
...
- [To:{my_account_id}] も [toall] も本文に含まれていない場合は必ず false（文脈推測・[rp to=] による代替判定は禁止）
- [To:{my_account_id}] が含まれている場合、以下のいずれかに該当する場合のみ true：
  - 今日中または明日中に自分が返答・対応する必要があるとわかるもの
  - 自分に関係する金額・契約・キャンセル・解約に関わる内容
  - 自分に関係するクレーム・トラブル・緊急対応が必要な内容
- [toall] のみが含まれている場合、以下のいずれかに該当する場合のみ true（[toall] の追加条件）：
  - 障害・システム停止・緊急対応等、全員に即時対応が求められる内容
  - 締切が今日中または明日中であり、全員への作業要求が明確な内容
  ※ 単なる全体周知・情報共有・報告・挨拶は false
- [To:{my_account_id}] / [toall] があっても、感謝・挨拶・確認の返信（「ありがとうございます」「承知しました」「よろしくお願いします」等）は false
