...
**削除ポリシー：** 実装が完了し、push済みのエントリは次回sync時に削除する。「作業なし」判定は残件セクションのみで行い、このログが残っていても「作業なし」にできる。

- [2026-03-30] ga4-report.py の広告着地URL検出を `/` ハードコードから動的検出に変更する（LP切り替えと同時に実施）（`scripts/ga4-report.py`）
