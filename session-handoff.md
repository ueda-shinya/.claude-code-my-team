...
[2026-04-07] /lp-create スキル ステップ⑤（デザイン設計）の深掘り（続きは別セッションで）

**現状：** ミオのリサーチをもとに大まかな項目を追加済み（`skills/lp-create/skill.ja.md` / `skill.md`）
- 追加済み項目：FV設計・視線誘導パターン・信頼要素配置・CTA設計・離脱防止・パフォーマンス設計方針
- マイクロコピーの担当分離（Kai：配置制約、コト：文言）を明記
- 承認ゲート②のフォーマット改善（FV構成・CTA配置・信頼要素配置を追加）

**深掘りが必要な項目（次セッションで着手）：**
- 各項目の具体的な指示粒度をどこまで書くか（Kai への渡し方の精度を上げる）
- FV設計仕様の詳細化（キャッチ・ビジュアル・CTAの配置パターンを例示するか）
- 視線誘導パターンの判断基準（どんな条件でZかFかを選ぶか）
- パフォーマンス設計のルール化（画像形式・lazy loading の判断フローを明記するか）
- ヒアリングシート（`skills/lp-create/hearing-sheet.md`）との整合確認（Phase 7 の素材情報がステップ⑤に正しく引き継がれているか）

**参考：** ミオのリサーチ結果はセッション履歴に残っている（2026-04-07セッション）
