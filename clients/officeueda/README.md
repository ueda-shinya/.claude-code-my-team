...
- **Web制作経験**: 4年（2026-04-28 時点）
- **制作実績**: サイト30以上
- **本職**: コーダー（Web制作）

※ 上記は対外的な実績訴求（戦略書・LP・営業資料・SNS）で出典として引用してよい一次記録。引用時は本セクションを出典として明記する（例: `clients/officeueda/README.md 実績・経歴セクション`）。
※ 数値更新時は本セクションを更新し、合わせて関連する成果物・戦略書の数値も差分点検する。
