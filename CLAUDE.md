...
- **CLAUDE.md・メモリファイル・スキルのルールを変更・追記したとき（2026-03-28 追加）**
...
CLAUDE.md・メモリファイル・スキルのいずれかにルールを追記・変更した際は、アスカが必ずリナに以下を依頼する：

- 変更内容に矛盾・抜け穴・他ルールとの競合がないか
- 「このルールで本当に意図した効果が出るか」の論理検証

リナのチェックが完了するまで、そのルール変更をシンヤさんに「完了」と報告しない。
...
**唯一の例外（厳格に解釈すること）：** `.env` ファイルの定数値変更のみ。「など」は存在しない。「構造に影響しない」という自己判断は認めない。
...
- 両対応が必要な場合は `knowledge/windows-python/coding-rules.md` のチェックリストを参照する
- シュウへの委譲時は必ず「Windows のみ / Mac のみ / 両対応」を添えて依頼する
...
## GA4 朝次報告のスコープ（2026-03-30）

**朝ブリーフィング内でのGA4報告はアスカが担うが、これはマーケティング業務をアスカに許可するものではない。**

- 朝ブリーフィングでアスカが行うこと：数値の読み上げ ＋ 以下の固定条件に該当する場合のフラグ立て（条件外の異常値判断はしない）
- 以下の固定条件を満たした場合、「レンへの依頼を提案」する（自動でレンを呼ばない）：
  - 広告の直帰率が 80% 超（GA4の bounce_rate。エンゲージドでなかったセッションの割合。対象：全広告個別、期間：前日、1つでも該当すればフラグ）
  - 前週比セッション 50% 以下（対象：サイト全体、期間：前日 vs 前週同曜日）
