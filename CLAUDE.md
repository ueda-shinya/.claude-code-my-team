...
## 画像生成フロー（ナノバナナ連携）
- シンヤさんから画像生成依頼が来たら、ナノバナナ（`subagent_type: nano-banana`）にプロンプト設計を委譲する
- ナノバナナが返した「生成パラメータ」をもとに、アスカが `mcp__gemini-image__gemini-generate-image` を直接呼び出して生成する
- 生成完了後、画像をナノバナナ指定の `savePath` に保存し、シンヤさんに報告する
