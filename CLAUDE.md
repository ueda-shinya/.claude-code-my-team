...
## Rina Pre-Review Gate (新規作成時の前段関門, Added 2026-05-02)

**目的**: 上位真因「問題顕在化駆動」への直撃対策。スキル/エージェント新規作成・大幅改修・外部DB/API依存処理の追加について、**実装着手前**にリナの事前レビュー関門を置き、運用順序を「事後レビュー駆動」から「事前検証駆動」へ反転する。

### 発動条件（離散基準・3カテゴリのいずれか1つ該当で発動）

1. **新規スキル作成**: `skills/` 配下に新規ディレクトリ＋SKILL.md を作成する場合
2. **新規エージェント作成**: `agents/` 配下に新規 `*.ja.md` / `*.md` ペアを作成する場合
3. **大幅改修**（既存スキル/エージェントの以下いずれか1つ該当）:
   - 機能追加（新規セクション・新規ステップ・新規モードの追加）
   - 外部依存追加（新規API呼び出し・新規DB参照・新規MCP連携の追加）
   - 入出力I/F変更（プロンプト引数の追加削除・出力フォーマットの構造変更）

### 対象外（事前レビュー不要）

- 既存 CLAUDE.md ルールの改訂（既存「Rina Auto-Invocation Rule」の事後レビューで担保）
- `knowledge/` `memory/` への追記（既存の事後レビュー継続）
- 文言修正・typo修正・既存ロジック維持の内部リファクタ
- バグ修正（機能追加・外部依存追加・I/F変更を伴わないもの）

**保守的判定原則**: 判定が曖昧な場合は「対象」側に倒す。アスカが「対象外」と判断した案件は、判断根拠を `memory/prereview-log.md` に記録し、月1で監査する。

### 事前レビュー観点（リナ用チェックリスト・4項目）

リナは以下4項目を**起案文書（要件定義 or SKILL.md ドラフト）**に対して検証し、1つでも欠落していればアスカへ差戻す（実装着手の運用ブロック）：

| # | 観点 | 検証内容 |
|---|---|---|
| ① | 実スキーマ/実仕様との照合方法 | 外部API/DB依存がある場合、実スキーマ確認手段（公式ドキュメント参照・サンプルレスポンス取得・実環境疎通確認等）が要件に明記されているか |
| ② | 既存同種実装への参照パス | 既存に同種スキル/スクリプトが存在する場合、その参照パスが要件に明記されているか（車輪の再発明防止）。**起案者は事前に `skills/` `scripts/` を最低1回 Grep し、検索クエリと結果（該当あり/なし）を起案文書に明記する**。リナはGrep結果の有無を検証する |
| ③ | 検知メカニズム | スキル/エージェントが期待通り動作しなかった場合、それを検知する手段（5状態マーカー / ログ出力 / 検証ステップ）が要件化されているか |
| ④ | セルフチェック項目 | 実装後に「正しく動いているか」を自己検証するチェック項目が要件段階で列挙されているか |

### 運用フロー

1. アスカが新規作成/大幅改修案件を起案（起案文書 or SKILL.md ドラフトを準備）
2. アスカが Agent tool 経由でリナに事前レビューを依頼（観点①〜④を明示）
3. リナが4観点を検証 → 通過 or 差戻し
4. 通過 → 実装着手（シュウ等に委任）
5. 差戻し → アスカが起案文書を補強 → 再度リナへ
6. 実装完了後、既存「Rina Auto-Invocation Rule」に基づく事後レビュー（CLAUDE.md/agents/skills 編集に対する従来の自動検証）を継続実施
7. 事前レビュー履歴は**リナの判定直後にアスカが** `memory/prereview-log.md` に追記（日付・対象案件・検証観点・通過/差戻し）。記録主体はアスカ、責務はアスカ単独
8. 通過後の実装委任時、アスカは依頼文に**「Pre-Review Gate 通過済み（YYYY-MM-DD）」マーカーを必ず含める**。シュウ/カナタは本マーカーなしの依頼を受託せずアスカに差戻す（違反検知二次経路の物理担保）

### 違反検知

- **一次検知**: アスカ自身（起案時のセルフチェック）
- **二次検知**: シュウ/カナタが実装委任を受けた際、依頼文に「事前レビュー通過済み」の明記がないものは受託せずアスカに差し戻す
- **三次検知**: 月1の `memory/prereview-log.md` 監査時にアスカが追跡

※ マーカー文言は運用フロー L8 の正規表記「**Pre-Review Gate 通過済み（YYYY-MM-DD）**」に従う（半角丸括弧・日付必須）。簡略表記・日付なしは差戻し対象。

### 禁止事項

- 事前レビューを経ずに新規スキル/エージェント作成・大幅改修の実装に着手すること
- 事前レビューでリナが差戻したまま実装に進むこと
- 「急ぎだから」「軽微だから」を理由に発動条件を恣意的に外すこと

### 既存ルールとの関係

- **既存「Rina Auto-Invocation Rule」**（事後レビュー）: CLAUDE.md/memory/agents/skills 編集後の事後検証 → **継続適用**（重複適用ではなく前後で挟む構造）
- **既存「Deliverable Quality Gate」**: 実装完了後のレビュアー振分け → **継続適用**（事前レビューはGate前段の追加関門）
- **既存「スキル出力の5状態契約」**: 観点③「検知メカニズム」と整合 → 対象スキルは5状態契約準拠を要件に明記

### 違反時の対応

- 事前レビュー未実施で実装着手したことが判明した場合、即時実装を停止
- アスカからシンヤさんに違反事実を報告
- `memory/feedback-dev-workflow.md` に違反履歴を追記
- 起案文書から作り直し、事前レビューに戻す

### Rina の自己検証問題について

本ルールはリナ自身の役割追加だが、検証対象は「ルール文の論理整合性」であり、リナの本来業務（論理検証）の範囲内。既存「Rina Auto-Invocation Rule」も同構造（リナが自分の発動条件を検証する運用）で稼働中のため、新規問題ではない。中立性が問題化した時点で別kaizen案件としてセカンドレビュー導入を検討する。

**ただし「既存が動いているから問題なし」は論証ではなく前例踏襲である**。本ルール運用後、自己検証バイアス（リナが自分の役割追加を甘く通す傾向）の有無を `memory/prereview-log.md` の差戻し率で観測し、3ヶ月後（2026-08-02 目処）に再評価する。
