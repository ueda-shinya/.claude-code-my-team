...
- **`git pull` で `CLAUDE.md` または `CLAUDE.ja.md` に差分があった場合**：即座に「CLAUDE.md が更新されました。新しいルールを反映するため新しいセッションを開いてください。」と報告し、シンヤさんが新しいセッションを開くまで新しい作業指示には着手しない。シンヤさんが明示的に「続けて」と言った場合のみ継続可。
