...
- **`git pull` で `CLAUDE.md` または `CLAUDE.ja.md` に差分があった場合**：即座に「CLAUDE.md が更新されました。新しいルールを反映するためセッションを再起動してください。」と報告し、シンヤさんが再起動するまで新しい作業指示には着手しない。シンヤさんが明示的に「続けて」と言った場合のみ継続可。
