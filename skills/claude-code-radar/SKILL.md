...
3. Notion DB に「通し番号」プロパティが存在するか（初回のみの確認）
   - 未確認時: `python ~/.claude/scripts/notion-radar.py --add-sequence-property` を一度実行すること
   - 冪等なので既存なら [INFO] を出して終了する
...
**通し番号（seq）のパース（オーケストレータ向け）:**

`notion-radar.py --add` の登録成功時の stdout は以下の形式で通し番号を返します：

```
[OK] 登録完了: #042 タイトル
```

登録成功（stdout が `[OK]` で始まる）した場合、正規表現 `#(\d+)` でこの番号をパースし、Step 8 の `star4plus_entries` の各要素に `seq` フィールド（整数）として格納してください。パース失敗時は `seq` フィールドを省略（後段がフォールバック処理を行う）。
...
```json
{
  ...
  "star4plus_count": 4,
  "star4plus_entries": [
    {"seq": 42, "stars": 5, "category": "MCP", "title": "...", "reason": "...", "url": "..."}
  ],
...
- **並行実行禁止**: `--add` の並行実行（cron実行中に手動実行など）は通し番号が重複採番される可能性があります。同時刻に複数実行しないこと。Cronは1日1回、手動実行は Cron と重ならない時間帯に限る
