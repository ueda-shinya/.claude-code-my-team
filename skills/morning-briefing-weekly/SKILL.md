...
- `TOPPAGE_SESSIONS_YD` が 0 → 「昨日トップページへのアクセスがありませんでした。」（BOUNCE・DURATIONは出力されないためスキップ）
...
- `LP_SESSIONS_YD` が 0 → 「昨日LPへのアクセスがありませんでした。」（BOUNCE・DURATIONは出力されないためスキップ）
- `LP_BOUNCE_YD` が 80% 超 → 「LP直帰率が高い（X%）。広告クリエイティブとLP訴求のズレを確認。」
- `LP_AVG_DURATION_YD` が 30秒未満 → 「LP平均滞在が短い（X秒）。ファーストビューの見直しを推奨。」

**LP 7日評価（`LP_ALERT` リスト）：**
- `LP_BOUNCE_7D` が 60% 超 → 「直帰率が高め（X%）。(1)SNS vs オーガニック別に流入元を確認 (2)モバイルをデバイス別で確認 (3)ランディングページレポートでセッション数の多いページを優先調査」
- `LP_MOBILE_BOUNCE_7D` が 70% 超 → 「モバイル直帰率が高め（X%）。スマホUI/UX問題・誤タップ・レイアウト崩れを確認。デバイスカテゴリでセグメント分割して原因を切り分けること」
...
ステップ4で取得した数値（SITE_SESSIONS / SITE_BOUNCE / SOURCE_* / LP_* / CONTACT_* / TOPPAGE_* など）をすべて渡す。

```
オフィスウエダのGA4データを渡します。
トップページとLPそれぞれについて、マーケティング観点から評価と対策を提案してください。
出力形式：
■ トップページ評価
・（箇条書き）
■ LP評価
...
【トップページ（/）昨日】
セッション: X / 離脱率: X% / 平均滞在: X秒

【LP（lp-260319）昨日】
セッション: X / 離脱率: X% / 平均滞在: X秒 / CTAクリック（累計）: X
