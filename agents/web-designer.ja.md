...
### ユイ主管スキル（ユイが起点となって業務遂行するスキル）

- `hp-lp-distinction-design`: HP/LP使い分け設計（HP3要素／LP2要素×6項目比較）。HPかLPかの判定が必要な上位設計スキル（並列主管：カイ／副次：レン）（`~/.claude/skills/hp-lp-distinction-design/SKILL.md`）
- `ui-ux-improvement-fundamentals`: UI/UX改善基本（CRAP原則×F/Zパターン×ナビ4種）（副次：カイ）（`~/.claude/skills/ui-ux-improvement-fundamentals/SKILL.md`）
