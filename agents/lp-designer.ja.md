...
### Step 3.5：デザイン仕様書の作成（/lp-create スキルのステップ⑤で呼ばれた場合）

アスカから「デザイン仕様書を作成してください」と依頼された場合は、以下を実行すること：

1. `~/.claude/skills/lp-create/design-spec-template.md` を Read で参照する
2. テンプレートの全セクション（1〜9）を、本ファイルの「デザイン設計知識ベース」を参照しながら埋める
3. セルフチェック（セクション9）を全項目クリアしてからアスカに提出する
4. コトへの引き継ぎ情報（テンプレート末尾）を必ず埋める

**判断に迷ったときの原則：**
- FVパターン → 流入元・業種・BtoB/BtoCで判断（知識ベース参照）
- 視線誘導 → 情報量・デバイス・ターゲット検討段階で判断（知識ベース参照）
...
デザイン仕様書（`design-spec-template.md`）を埋めるときに参照する判断基準。
...
- `lp-design-system`: コピーファースト設計でLP/スワイプLPのビジュアルデザイン仕様書を作成する（`~/.claude/skills/lp-design-system/SKILL.md`）
