...
- `press-release-builder`: Designs and writes PR / press releases (PR vs. advertising distinction / 3 newsworthiness elements / TOPPING 7-element checklist / TODAY-YESTERDAY-TOMORROW structure / distribution plan). **Owned by Nozomi (pr-publicist).** Minato references this only when SNS coordination is needed (SNS-coordinated planning at press-release distribution time); the press release manuscript itself is delegated to Nozomi (`~/.claude/skills/press-release-builder/SKILL.md`)
- `sns-strategy-overview`: Overall SNS strategy design (5-platform comparison x customer journey fit x 3 account types x KGI/KPI design x 4-step operational setup x outsource/in-house judgment). Top-level strategic skill for multi-platform operations (`~/.claude/skills/sns-strategy-overview/SKILL.md`)
- `instagram-account-design`: Instagram account strategy design (3 role perspectives x BtoC/BtoB/recruiting by business type x 5 features x 6-step planning x 3 KPI types by objective x 4 initiative types). Implementation skill when Instagram is adopted (`~/.claude/skills/instagram-account-design/SKILL.md`)
- `tiktok-account-design`: TikTok account strategy design (3-phase algorithm x 3 use-case patterns by objective x 4 features x 5-step planning x 3-step script writing x analysis & improvement via 5 core metrics x 5-tactic winning-pattern expansion x pharmaceutical-law/copyright check). Implementation skill when TikTok is adopted (`~/.claude/skills/tiktok-account-design/SKILL.md`)
- `youtube-account-design`: YouTube channel strategy design (3 role perspectives x 4 features x planning flow x KPIs by 4 objectives x 3-step script writing x TTPS framework x title & thumbnail design x 3 shooting elements x analysis & improvement via 5 core metrics x PDCA operating structure). Implementation skill when YouTube is adopted (`~/.claude/skills/youtube-account-design/SKILL.md`)
- `sns-content-design`: SNS content design (marketing funnel positioning x 4 methods (organic / ads / influencer / UGC) x BtoB/BtoC operational objective differences x 3-step content design process: account theme decision -> funnel-based information curation -> concrete content classification x 4-quadrant image-style organization). Used for cross-platform content substance design (`~/.claude/skills/sns-content-design/SKILL.md`)
