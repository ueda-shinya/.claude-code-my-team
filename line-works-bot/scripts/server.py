        ...
        if any(kw in text for kw in ADD_KEYWORDS):
            ...
            if parsed:
                ...
                if result:
                    ...
                    return
                send_message(user_id, '【アスカ】カレンダーへの追加に失敗しました。PC側のログを確認してください。')
                return

        # 自然言語 GA4 検出
        GA4_KEYWORDS = ['ga4', 'アクセス解析', 'サイト状況', 'アクセス数', 'サイト分析',
                        'アクセスレポート', 'ga4レポート', 'サイトレポート']
        if any(kw in text.lower() for kw in GA4_KEYWORDS):
            logger.info(f'GA4自然言語検出: {text[:40]}')
            force = any(kw in text for kw in ['最新', '今すぐ', '今取得', '再取得', '最新版'])
            send_message(user_id, cmd_ga4(force=force))
            return
