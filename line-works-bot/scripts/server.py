...
def process_message(user_id: str, text: str, channel_id: str = None, received_bot_id: str = None):
    try:
        ...
        if any(kw in actual_text for kw in ADD_KEYWORDS):
            ...
            if parsed:
                ...
                if result:
                    ...
                    return
                send_message(user_id, '【アスカ】カレンダーへの追加に失敗しました。PC側のログを確認してください。', channel_id=channel_id, bot_id=received_bot_id)
                return
        ...
        error_prefix = '【ミオ】' if (MIO_BOT_ID and received_bot_id == MIO_BOT_ID) else '【アスカ】'
        send_message(user_id, f'{error_prefix}申し訳ありません。処理中にエラーが発生しました。PC で確認してください。', channel_id=channel_id, bot_id=received_bot_id)
...
def callback():
    ...
    def run():
        if not channel_id:  # DM のみ「受け付けました」を送信
            ack_prefix = '【ミオ】' if verified_bot == 'mio' else '【アスカ】'
            send_message(user_id, f'{ack_prefix}受け付けました。少々お待ちください...', bot_id=received_bot_id)
