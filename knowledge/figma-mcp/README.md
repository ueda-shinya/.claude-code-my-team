...
## プラン別制約（2026-03-19 確認）

| 接続方式 | 必要プラン | 備考 |
|---|---|---|
| デスクトップMCP（`127.0.0.1:3845`） | **有料プラン必須**（Professional以上） | Dev Modeが有料機能のため無料プランでは設定項目自体が表示されない |
| リモートMCP（`mcp.figma.com`） | 要検証 | 無料プランでも使える可能性あり。月6回制限 |

- **無料プランでデスクトップMCPを試みても設定項目が見つからない** → 有料プラン（Professional: 約$16/月）へのアップグレードが必要
- 評価目的なら先にリモートMCPを試す方が低コスト

---

## 導入手順（最短 PoC）

1. Figma デスクトップで Dev Mode → MCP 有効化（**有料プラン必須**）
