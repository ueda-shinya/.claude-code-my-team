...
<body id="home">
	...
	<main id="wrapper">
		...
		<section>
			...
			<div class="cotactform-wrap scrFadeTop">
				<form method="post" class="contactform" action="confirm/?g=<?php echo time() ?>" name="contactform" id="contactform-form">
					<div class="form-row">
					...
					<div class="form-row">
						...
						<input type="tel" class="validate[required,funcCall[validateJpTel]]" id="tel" name="tel" placeholder="電話番号">
					</div>
	...
	<script>
		...
		// ValidationEngine の funcCall[validateJpTel] から呼び出される
		// @param {jQuery} field - 対象フィールドの jQuery オブジェクト
		// @param {Array} rules  - ルール配列（ValidationEngine から渡される）
		// @param {number} i     - ルールのインデックス
		// @param {Object} options - ValidationEngine のオプション
		// @returns {true|string} 通過時は true、エラー時はエラーメッセージ文字列
		// -----------------------------------------------------------------------
		function validateJpTel(field, rules, i, options) {
			var value = $.trim(field.val())
			if (value === '') return true // 必須チェックは required に任せる
			if (!/^[0-9\-]+$/.test(value)) {
		...
		function initValidate() {
			0 < $("#contactform-form").length && $("#contactform-form").validationEngine({
