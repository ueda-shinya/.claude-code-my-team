...
| バージョン | 日付 | 変更内容 |
|---|---|---|
| v1.6.2 | 2026-04-29 | bornstem_contact 案件改善のテンプレバックポート（4件）。(1) `form.js` に `scrollToFormMessage()` 関数を追加し、バリデーションエラー・AJAXサーバーエラー・通信エラー・`?error=1` フォールバックの4経路でスクロール発火。`DOMContentLoaded` 内に `?error=1` 検出ロジック追加。(2) `showFormMessage()` 内の `scrollIntoView()` 呼び出しを削除し、スクロール責任を `scrollToFormMessage()` に移管（責任分離）。(3) `contact.php` の `#form-message` 表示を if/else 構造に変更（noJSエラー時は `id="form-message"` + `c-form-message--error`、通常時は `id="form-message"` + `hidden`）。id が1箇所にしか出ない構造を明示的に保証。(4) `contact.php` / `thanks.html` の静的アセット読み込みにキャッシュバスター `?v=20260429` を付与（`form.css` / `form.js` / `site.css`）。 |
| v1.6.1 | 2026-04-28 | bornstem_contact 案件発見の問題をテンプレにバックポート（4件）。(1) `submit.php` レートリミット fail-open ログに `[SECURITY-ALERT]` タグ＋「fail-open発生」文言を追加。(2) `submit.php` 自動返信メール失敗時のユーザー向け文言を緩和分岐（AJAX経路）。(3) `contact.php` ハニーポット `<input>` に `aria-hidden="true"` を追加（二重防御）。(4) `templates/.htaccess` 冒頭に説明コメント2行を追加（deny all 設定は維持）。 |
